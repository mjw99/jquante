...
public class ContractedGaussian {
	...
	private void overlapDerivativeHelper(PrimitiveGaussian iPG, PrimitiveGaussian jPG, Vector3D pOrigin,
			Vector3D ovrDer) {
	...
	public CompactContractedGaussian toCompactContractedGaussian() {

		double[] primativeExponents = this.exponents.stream().mapToDouble(d -> d).toArray();
		double[] primativeCoefficients = this.coefficients.stream().mapToDouble(d -> d).toArray();
		double[] primativePrimnorms = this.primNorms.stream().mapToDouble(d -> d).toArray();

		return new CompactContractedGaussian(this.powers.getL(), this.powers.getM(), this.powers.getN(),
				this.origin.getX(), this.origin.getY(), this.origin.getZ(), this.getNormalization(), primativeExponents,
				primativeCoefficients, primativePrimnorms);
	}
	...
	public String toString() {
		return "Origin : " + origin + " Powers : " + powers + " Normalization : " + normalization + " Primitives : "
				+ primitives;
	}
}