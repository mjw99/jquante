 ...
 * use individual objects of this class in separate threads.
 * 
 * @author V.Ganesh
...
public class MoleculeBuilderImpl extends MoleculeBuilder {
	...
	protected double[] covalentRadius, vdwRadius, weakBondAngle,
			dblBndOverlaps;
	 ...
	 * algorithms for identifying the required type of bonds. Also the
	 * implementation should *not* worry about how the connectivity is to be
	 * stored, this is taken care of by the implimentors of Molecule class. <br>
	 * 
	...
	public void makeConnectivity(Molecule molecule, List<Integer> atomIndices) {
		...
		for (Integer atomIndex : atomIndices) {
			try {
				...
				dblBndOverlaps[atomIndex] = atomInfo
						.getDoubleBondOverlap(symbol);
	...
	public boolean canFormBond(Atom a1, Atom a2, BondType bondtype) {
		...
		if (!canFormBond(a1.getAtomCenter(), a2.getAtomCenter()))
			return false;
	...
	protected boolean isOrientedProperly(int atomIndex1, int atomIndex2) {
		...
		vectorFrom1To2 = new Vector3D(atom2.getAtomCenter().sub(
				atom1.getAtomCenter()));
		vectorFrom2To1 = new Vector3D(atom1.getAtomCenter().sub(
				atom2.getAtomCenter()));
	 ...
	 * 
	 * @param atom
	 *            - the atom to be used for defining the axis
	 * @param atomIndex
	 *            - and the related atom index
	...
	public void makeZMatrix(Molecule molecule) {
		...
		for (int i = 0; i < molecule.getNumberOfAtoms(); i++) {
			...
			for (j = i; j < molecule.getNumberOfAtoms(); j++) {
				if (molecule.getAtom(j).getIndex() == i)
					break;
			} // end for
		...
		for (; atoms.hasNext();) {
			...
			if (atom.getIndex() == 1) {
				...
				if (molecule
						.isBonded(atom.getIndex(), referenceAtom.getIndex())) {
					atom.setLengthReference(new ZMatrixItem(referenceAtom, 0.0));
					atom.setAngleReference(new ZMatrixItem(referenceAtom
							.getLengthReference().getReferenceAtom(), 0.0));
					atom.getDihedralReference().setReferenceAtom(null);
				} else {
					referenceAtom = molecule.getAtom(0);
					atom.setLengthReference(new ZMatrixItem(referenceAtom, 0.0));
					atom.setAngleReference(new ZMatrixItem(molecule.getAtom(1),
							0.0));
					atom.getDihedralReference().setReferenceAtom(null);
				} // end if
			...
			while (atomsN.hasNext()) {
				...
				if ((atom.getIndex() < 2) && (atomN.getIndex() < 2))
					continue;

				if (atom.getIndex() == atomN.getIndex())
					break;
			...
			if (referenceAtom.getIndex() == 0) {
				atom.setAngleReference(new ZMatrixItem(molecule.getAtom(1), 0.0));
			} else {
				atom.setAngleReference(new ZMatrixItem(referenceAtom
						.getLengthReference().getReferenceAtom(), 0.0));
			} // end if
			...
			if (referenceAtom.getIndex() <= 1) {
				atom.setDihedralReference(new ZMatrixItem(molecule.getAtom(2),
						0.0));
			} else {
				atom.setDihedralReference(new ZMatrixItem(referenceAtom
						.getAngleReference().getReferenceAtom(), 0.0));
			} // end if
		} // end for
		...
		for (int k = 1; k < molecule.getNumberOfAtoms(); k++) {
			...
			// dihedral
			atom.getDihedralReference().setValue(
					MathUtil.toDegrees(MathUtil.findDihedral(
							atom.getAtomCenter(), a.getAtomCenter(),
							b.getAtomCenter(), c.getAtomCenter())));
		} // end for
	...
	protected boolean preProcessMolecule(Molecule molecule) {
		try {
			// detect every thing
			if ((Boolean) molecule
					.getCommonUserDefinedProperty(CommonUserDefinedMolecularPropertyNames.DETECT_ALL) == true) {
				return false;
			} // end if
		} catch (Exception ignored) {
		}
		...
		try {
			...
			if ((Boolean) molecule
					.getCommonUserDefinedProperty(CommonUserDefinedMolecularPropertyNames.DETECT_ONLY_SINGLE_BONDS) == true) {
				makeSimpleConnectivity(molecule);
		...
		try {
			...
			if ((Boolean) molecule
					.getCommonUserDefinedProperty(CommonUserDefinedMolecularPropertyNames.DETECT_ONLY_SPECIAL_STRUCTURE) == true) {
				identifySpecialStructures(molecule);
		...
		try {
			// detect weak bonds
			if ((Boolean) molecule
					.getCommonUserDefinedProperty(CommonUserDefinedMolecularPropertyNames.DETECT_ONLY_WEAK_BONDS) == true) {
		...
		try {
			...
			if ((Boolean) molecule
					.getCommonUserDefinedProperty(CommonUserDefinedMolecularPropertyNames.DETECT_ONLY_MULTIPLE_BONDS) == true) {
